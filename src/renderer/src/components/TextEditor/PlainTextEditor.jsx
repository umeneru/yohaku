...
const PlainTextEditor = forwardRef(function PlainTextEditor({ warning = null }, ref) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        dispatch({ type: 'SAVE_FILE', filePath: currentFile, content })
      } catch (err) {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === 'Control' || e.key === 'Meta') setIsModifierPressed(true)
    ...
    <div className={styles.editor}>
      ...
      <div ref={containerRef} className={styles.highlightContainer}>
        ...
        {urlOverlayContent && (
          <div
            ref={urlOverlayRef}
            className={`${styles.urlOverlay}${isModifierPressed ? ` ${styles.urlOverlayInteractive}` : ''}`}
